...
with margin rescaling: $min_{w, \xi \geq 0} \frac{1}{2}w^Tw + C \sum_{i=1}^n \xi_i$ st. $w^T\mathbf{\psi}(z_i,\mathbf{y_i})-max_{z \neq z_i}[\delta(z,z_i)+w^T\mathbf{\psi}(z,\mathbf{y_i})] \geq - \xi_i$
